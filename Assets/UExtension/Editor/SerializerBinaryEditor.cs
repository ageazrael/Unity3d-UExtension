...
namespace UExtension
{
    ...
    {
		const string GeneratePathRoot       = "Assets/UExtension.Generate/SerializerBinary/";
        ...
        public override void CSGenerateProcess(CSGenerate rGenerate)
        {
            ...
            foreach (var rType in SerializerBinaryTypes.Types)
            {
                ...
                rText
                    .F("public partial class {0}", rType.FullName.Replace(rType.Namespace + ".", string.Empty).Replace('+', '.')).N()
                    .A("{").N();
                ...
                rGenerate.Add(rText.ToString(), PathExtension.Combine(GeneratePath, rGroupName, rType.FullName.Replace('+', '.') + ".Binary.cs"));
        ...
        List<MemberInfo> SearchSerializeMember(Type rType)
        {
            ...
            foreach(var rMemberInfo in rType.GetMembers())
            {
                ...
                {
                    Debug.LogErrorFormat("{0}.{1} Skip Serialize!", rType.FullName, rMemberInfo.Name);
                    continue;
                }
